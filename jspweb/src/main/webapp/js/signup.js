...
$(function(){	//문서 열리면 해당 코드가 실행
	...
	$("#mid").keyup( function(){	//mid가 입력될때마다 해당 함수 실행
		...
		// 2. HTML 태그내 데이터 넣기
		//document.getElementById("idchack")
			//		.innerHTML = "사용가능한 아이디 입니다.";
		...
		let idj = /^[a-zA-Z0-9]{5,15}$/;	//한글을 제외한 영문+숫자 5~15 사이의 문자열
			...
			//[A-Z] : 대문자 찾음
			//[0-9] : 숫자 찾음
			//{ 최소길이 , 최대길이 } : 문자 최소길이~최대길이 까지만 입력 받겟다
				//정규표현식.test(변수) : 해당 변수가 정규표현식에 동일하면 true 다르면false
	...
	$("#memail").keyup( function(){
		...
		if( memailaddress == "" ){ 
			...
			if(emailj.test(memail)){
				...
				$("#emailcheck").html("이메일형식이아닙니다");pass[5]=false;
			}
		}
	});
	//이메일주소 목록상자 선택시
	$("#emailselect").change( function(){//목록상자내 값이 변경되었을떄
		let emailselect = $("#emailselect").val();
		if(emailselect ==""){
			$("#memailaddress").val("");
			$("#memailaddress").attr("readonly", false); //읽기모드취소
		}else{
			$("#memailaddress").val(emailselect);	// val() -> value
			$("#memailaddress").attr("readonly", true);//attr -> attribute 속성을 넣어줌
		}
...
//폼전송메소드
function signup(){//pass배열이 모두 true이면 폼 전송함
	//alert("하하하하")
	//console.log()
	let check = true;
	...
	}else{alert("필수입력 사항이 입력되지 않았습니다.")}
