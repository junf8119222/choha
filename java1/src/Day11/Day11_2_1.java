...
public class Day11_2_1 {
	...
	//p.422
		...
		//종류: 1.일반예외 2.실행예외
			...
			//실행예외: 컴파일[실행]후에 예외 검사[개발자 경험의 판단해서 예외처리]
		//예외처리
			//목적: 에러가 발생하면 프로그램은 종료된다.[안전성에 문제]
	...
	try {
		...
		System.out.println(data.toString());//object클래스 메소드 : toString() : 객체정보출력
			//실행전에는 에러 없다[실행 예외]
		//에러가 발생하는 이유: null은 메모리 할당이 없기 때문에 객체정보출력 불가능
		...
		int value2 = Integer.parseInt(data2);//"a100"->X[a는 정수로 표현 불가능]
					//integer.paseInt(문자열); 문자열-> 정수
		//오류 발생 이유: 문자열내 숫자형식은 변환이 가능하지만 문자열내 변환이 불가능
